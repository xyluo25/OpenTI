...
config_path = r"C:/Users/xyluo25/anaconda3_workspace/001_GitHub/OpenTI/OpenTI/pivotalAgent/Configs/path.yaml"
api_config = r"C:/Users/xyluo25/anaconda3_workspace/001_GitHub/OpenTI/OpenTI/pivotalAgent/Configs/config.yaml"
...
with open(config_path, 'r') as file:
    ...
    tsc_root = yaml_data["LibSignal"]["tsc_root"]
    filter_store_base = yaml_data["FIlter_Store_Base"]
    openTI_root = yaml_data["openTI_root"]
        ...
        def get_bounding_box(query):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Failed to get a response. Status code: {response.status_code}")
...
class GoogleMapDownloader:
    ...
    def getXY(self):
        ...
        top_left_x = (tile_size / 2 + self._west * tile_size /
                      360.0) * num_tiles // tile_size
        ...
        bottom_right_x = (tile_size / 2 + self._east *
                          tile_size / 360.0) * num_tiles // tile_size
...
class showMap:
    ...
    @func_prompt(name="showOnMap",
                 description="""
                 This tool is used to show th  # sourcery skip: avoid-builtin-shadow  # sourcery skip: avoid-builtin-shadowe interested loaction on Map, like the ASU campus area on map.
    ...
    def embody(self, target: str) -> str:
        ...
        def create_and_show_map(bounding_box):
            min_lat, max_lat, min_lon, max_lon = bounding_box[
                0], bounding_box[1], bounding_box[2], bounding_box[3]
            # Center of the map
            center_lat = (min_lat + max_lat) / 2
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to get the map. Status code: {response.status_code}")
        except Exception:
            return "Error for finding the place"

        _map = create_and_show_map(target)
        _map.save(html_path)
...
class autoDownloadNetwork:
    ...
    def embody(self, target: str) -> str:
        ...
        # try:
        print(f"get the data in download: {target}")
        desired = target.replace(" ", "").split(
            ",")[-1].replace('"', '').replace("[", "").replace("]", "")
        print("desired:"+desired)
        min_long, min_lat, max_long, max_lat = target.replace(
            "[", "").replace("]", "").strip().replace(" ", "").split(",")[:4]
        print("long_min, lat_min, long_max, lat_max")
        print(min_lat, max_lat, min_long, max_long)
        url = f"https://www.openstreetmap.org/api/0.6/map?bbox={min_long}%2C{min_lat}%2C{max_long}%2C{max_lat}"
        print("url:")
        ...
        if response.status_code == 200:
            ...
            if not os.path.exists(file_path):
                return "The requested cannot be successfully downloaded."
        else:
            file_path = 'None'
        ...
        time.sleep(2)
        return (f"The requested have successfully downloaded and saved at: {file_path}." +
                "And your final answer should include this sentence without changing anything:" +
                f"The file saved location is at: `{file_path}`.")
    ...
    def embody(self, target: str) -> str:
        ...
        if simulator.lower() == "cityflow":
            if algorithm.lower() in support_algorithms_1:
                ...
                log_file_relative_path = "data/output_data/tsc/cityflow_{}/cityflow1x1/0/logger/{}_DTL.log".format(
                    algorithm, timestamp)
                ...
                completed_process = subprocess.run(
                    ["python3", base_path+"run.py", "-a", algorithm, "-tt", timestamp], capture_output=True, text=True, cwd=base_path)
                ...
                saved_image = painter({'hz1x1': read_path}, [
                                      'epoch', 'average travel time', 'rewards', 'delay'])
                ...
                subprocess.run(["python3", self.base_dir+"run.py", "-a", algorithm,
                               "-tt", timestamp], capture_output=True, text=True, cwd=libsignal_root)

                sub_half = simulator + "_" + algorithm + \
                    "/cityflow1x1/0/logger/example_BRF.log"
...
class generateDemand:
    ...
    def embody(self, target: str) -> str:
        ...
        print(f"demand target:{target}")
        name = target.replace(".osm", "").split("/")[-1]
        dump_dir = openTI_root + "pivotalAgent/Data/demand/" + name
        net = og.getNetFromFile(target, network_types=(
            'walk', 'auto'), POI=True, default_lanes=True, default_speed=True)
        ...
        updated_dict = gd.sync_geometry_between_zone_and_node_poi(
        ...
        zone_od_distance_matrix = gd.calc_zone_od_distance_matrix(
            zone_dict_update)
        ...
        zone_prod_attr = gd.calc_zone_prod_attr(
            node_prod_attr, zone_dict_update)

        # Run gravity model to generate agent-based demand
        df_demand = gd.run_gravity_model(
            zone_prod_attr, zone_od_distance_matrix)

        # Generate agent-based demand
        df_agent = gd.gen_agent_based_demand(  # noqa: F841
...
class simulateOnDLSim:
    ...
    Please try to provide log info in bullet list or raw data, even though it is a large file.
    """)
    def embody(self, target: str) -> str:
        target = "./AugmentUtils/simulation/simulate/"
        print(f"DLSim target:{target}")
        try:
        ...
        return ("You have successfully simulated on DLSim." +
                "And your final answer should include this sentence without changing anything: " +
                f"The simulation process and logs are saved below: `{save_to}`, the key steps as: {output}.")
...
class visualizeDemand:
    ...
    def embody(self, target: str) -> str:
        ...
        return ("You have successfully visualized the traffic demand information. " +
                "And your final answer should include this sentence without changing anything: " +
                f"The traffic demand information at ASUtempe as below: `{base_path}`.")
...
class log_analyzer:
    ...
    This is the tool used to analyze the log files and provide comparison,
    if you are asked to analyze any given location files, please try your best to find the data,
    and provide logical and rational understanding on it.
    The output will tell you whether you have finished this command successfully.
    """)
    def embody(self, target: str):
        try:
            ...
            with open(file=path, mode="r") as content:
                log_data = content.read()
            return "You have the content read now, please provide your own understanding and provide the explained result of it, the content is: " + log_data
        except Exception:
...
class oneClickSUMO:
    def __init__(self) -> None:
        self.pipline_path = openTI_root + "/pivotalAgent/AugmentUtils/one_click_SUMO/"

    @func_prompt(name="oneClickSUMOSimulation", description="""
    This is the tool used to execute one click run SUMO simulation,
    if you are asked to run sumo simulation with one click,
    please consider using this tool. Before using, please check for an osm file path.
    The output will tell you whether you have finished this command successfully.
    """)
    def embody(self, target: str):
        # try:
        ...
        if osm_file_path is None:
            return ("Your final answer should include this sentence without changing anything: " +
                    "You are expected to input your osm file path (absolute path) to run this simulation!")
        # subprocess.run(["python3", current_path+ self.pipline_path + "simulate_pipline.py", "-f", target],
        # capture_output=True, text=True, cwd=current_path+ self.pipline_path)
        ...
        subprocess.run(["python3", sumo_pipline, "-f", target],
                       capture_output=True, text=True, cwd=current_path + self.pipline_path)
...
class odMatrixTest:
    ...
    def embody(self, target: str):
        try:
            ...
            subprocess.run(["python3", current_path + "pickout_test_direct.py"],
...
def vialize_od_results(file_path):
    ...
    ax.bar([i - 0.2 for i in indexes], volume, width=0.4,
           label='Ground Truth Volume', align='center')
    ax.bar([i + 0.2 for i in indexes], obs_count, width=0.4,
           label='OD Matrix Count', align='center')
...
class visualizeODResult:
    ...
    This is the tool used to visualize the OD matrix estimation results from DTALite simulator,
    If user ask about visualize the OD results of specific file path of DTALite execution,
